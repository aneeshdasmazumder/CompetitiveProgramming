...
/*
Question:
Given an integer array, sort the array in ascending order using Quick Sort.

Quick Sort chooses a pivot, partitions the array so smaller elements come before
the pivot and larger elements come after it, then recursively sorts both sides.
...
Space Complexity:
O(log n) average recursion stack space.
...
public class QuickSort {
    ...
    private static int partition(int[] arr, int low, int high) {
        ...
        while(low < arr.length && high >=0 && low < high) {
            while(low < arr.length && arr[low] <= arr[pi]) low++;
            while(high >=0 && arr[high] > arr[pi]) high--;
